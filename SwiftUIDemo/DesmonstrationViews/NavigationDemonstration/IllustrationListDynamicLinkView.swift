//
//  IllustrationListDynamicLinkView.swift
//  SwiftUIDemo
//
...
struct IllustrationListDynamicLinkView : View {
	...
	let illustrations = [
		...
		Illustration(id: "e", image: "profile-pic-2", thumbnail: "sushi-hug", title: "Sushi Time", intro: "Octopus ramen and sushi, which one ?"),
	...
	let destination = DynamicNavigationDestinationLink(id: \Illustration.id) { IllustrationView(image: $0.image) }
	
    var body: some View {
		
		return List {
			ForEach(illustrations.identified(by: \.id)) { illustration in
				NavigationButton(destination: self.destination, presenting: illustration) {
